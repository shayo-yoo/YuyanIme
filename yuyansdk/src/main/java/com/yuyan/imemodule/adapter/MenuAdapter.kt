...
class MenuAdapter (context: Context?, val data: MutableList<SkbFunItem>) : RecyclerView.Adapter<MenuAdapter.SymbolHolder>() {
    ...
    override fun onBindViewHolder(holder: SymbolHolder, position: Int) {
        ...
        val isVoiceMode = item.skbMenuMode == SkbMenuMode.Voice || item.skbMenuMode == SkbMenuMode.VoiceB
        val baseColor = if (isSettingsMenuSelect(item)) mTheme.accentKeyBackgroundColor else mTheme.keyTextColor
        val color = if (isVoiceMode && VoiceUiState.isRecording) Color.parseColor("#E53935") else baseColor
