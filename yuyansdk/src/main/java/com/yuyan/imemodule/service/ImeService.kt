...
import com.yuyan.imemodule.hotword.HotwordEngine
import com.yuyan.imemodule.hotword.PhoneticMatcher
...
class ImeService : InputMethodService() {
    ...
    internal lateinit var voiceController: VoiceController

    /** 热词引擎：从公共词库目录懒加载；未配置则保持 null（识别结果原样上屏） */
    private val hotwordEngine: HotwordEngine? by lazy {
        try {
            val root = GlossaryStore.defaultRootDir() ?: return@lazy null
            val suite = GlossaryStore.loadSuite(root) ?: return@lazy null
            if (!suite.isUsable) return@lazy null
            val engine = HotwordEngine(suite)
            // M3：热词用发音相似度匹配（阈值可配置文件调）
            engine.hotwordMatcher = PhoneticMatcher(suite.config.similarityThreshold)
            engine
        } catch (t: Throwable) {
            null
        }
    ...
    /**
     * 语音识别结果上屏的统一入口。
     * M1：直接上屏；M2/M3：在此处插入热词/正则/发音相似度替换引擎。
     */
    private fun commitVoiceResult(text: String) {
        // 热词替换可能涉及长文本计算，放后台线程做，避免卡输入法主线程(ANR)
        CoroutineScope(Dispatchers.Main).launch {
            val finalText = try {
                withContext(Dispatchers.Default) { hotwordEngine?.process(text) ?: text }
            } catch (t: Throwable) {
                text
            }
