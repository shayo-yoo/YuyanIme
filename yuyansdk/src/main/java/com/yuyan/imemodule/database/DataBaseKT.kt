        ...
        private fun initPhrasesDb() {  //初始化常用语数据数据
            ...
            if(instance.skbFunDao().getAllMenu().isEmpty()) {
                val skbFuns = listOf(
                    ...
                    SkbFun(name = SkbMenuMode.Voice.name, isKeep = 0, position = 16),
                    SkbFun(name = SkbMenuMode.VoiceB.name, isKeep = 0, position = 17),
                )
                instance.skbFunDao().insertAll(skbFuns)
            }
            // 老版本数据库升级：确保语音A/B 两个入口存在于“可用功能池”，并默认放进功能栏
            if (instance.skbFunDao().getAllMenu().none { it.name == SkbMenuMode.Voice.name }) {
                instance.skbFunDao().insert(SkbFun(name = SkbMenuMode.Voice.name, isKeep = 0, position = 16))
            }
            if (instance.skbFunDao().getAllMenu().none { it.name == SkbMenuMode.VoiceB.name }) {
                instance.skbFunDao().insert(SkbFun(name = SkbMenuMode.VoiceB.name, isKeep = 0, position = 17))
            }
            if (instance.skbFunDao().getBarMenu(SkbMenuMode.Voice.name) == null) {
                instance.skbFunDao().insert(SkbFun(name = SkbMenuMode.Voice.name, isKeep = 1))
            }
            if (instance.skbFunDao().getBarMenu(SkbMenuMode.VoiceB.name) == null) {
                instance.skbFunDao().insert(SkbFun(name = SkbMenuMode.VoiceB.name, isKeep = 1))
            }
        }
    }
}
